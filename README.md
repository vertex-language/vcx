...
- **What is not there yet.** `__managed__`, `__half`, textures, dynamic parallelism, `-rdc`, `printf` on AMD (ROCm's hostcall), and toolkit discovery (a machine with the CUDA toolkit or ROCm installed is not yet asked for its headers and libraries; vcx's own stand in). A HIP program compiles through both passes and its code object disassembles with `llvm-objdump`; it has not been run, since no AMD GPU has been near this code.
