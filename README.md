...
- **nvcc as the oracle.** Where nvcc and cl.exe are installed, every kernel of `tests/cuda/device` is also compiled by nvcc to a cubin and run through the same harness, and every program of `tests/cuda/host` is built by nvcc and run: the headers' expectations are what NVIDIA's compiler produces, and the two compilers agree on every one.
...
- **What is not there yet.** `__managed__`, `__half`, textures, dynamic parallelism, `-rdc`, `printf` on AMD (ROCm's hostcall), and the toolkit's headers: vcx's own `cuda_runtime.h` stands in even when a toolkit is installed, since the toolkit's pulls in `<functional>` and the rest of the STL, which vcx does not compile yet -- the front-end conformance work ahead. A HIP program compiles through both passes and its code object disassembles with `llvm-objdump`; it has not been run, since no AMD GPU has been near this code.
