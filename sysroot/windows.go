...
// winrt and cppwinrt (C++-only, but vcx opens only what is #include'd, so
...
// gets — vcx finds them the way the platform's own tools do, and the rest
...
// question with, so honoring it means vcx composes inside one, and a
// caller who set it by hand has said which headers to use.
//
// What vcx finds itself is appended rather than substituted, because
...
func windowsEntries(h Host) (entries []Entry, notes []string) {
	...
	if tc.msvc == "" {
		notes = append(notes,
			"no MSVC toolset found: install the Visual Studio Build Tools with the C++ workload, or run vcx from a Developer Command Prompt")
	}
	if tc.sdkInclude() == "" {
		notes = append(notes,
...
// applied to the other list: %LIB% first, then what vcx finds, deduplicated.
//
...
// %INCLUDE% and vcx's own walk name the same directories by construction,
