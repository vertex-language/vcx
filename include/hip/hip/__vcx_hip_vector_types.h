/*
 * __vcx_hip_vector_types.h -- the HIP vector types and dim3, laid out
 * as ROCm's hip_vector_types.h lays them out: the same names, the
 * same alignments, the same constructors. It claims the toolkit
 * header's guard so that the two are never both read.
 */
#ifndef __VCX_HIP_VECTOR_TYPES_H__
#define __VCX_HIP_VECTOR_TYPES_H__
